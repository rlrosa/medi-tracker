...
async function main() {
  ...
  console.log('--- VERIFY UPCOMING FIX ---');
  medications.forEach(med => {
    med.schedules.forEach(schedule => {
      let currentDue = schedule.startDate ? new Date(schedule.startDate) : new Date(startDate.getTime() - 24 * 60 * 60 * 1000);
      ...
      console.log(`\nMed: ${med.name} | Sch: ${schedule.name}`);
      while (currentDue && currentDue <= futureLimit) {
        ...
        if (currentDue < now || currentDue <= futureLimit) {
